...
static void add(Container &c, uint64_t val) {
    ...
    HeapNode item;
    item.pos_ref = &d->heap_idx;
    item.val = val;

    c.heap.push_back(item);
    heap_fix(c.heap, c.heap.size() - 1);
}
...
static void del(Container &c, uint64_t val) {
    ...
    assert(c.heap.at(d->heap_idx).pos_ref == &d->heap_idx);
    c.heap[d->heap_idx] = c.heap.back();
    c.heap.pop_back();
    if (d->heap_idx < c.heap.size()) {
        heap_fix(c.heap, d->heap_idx);
    }
    delete d;
    c.map.erase(it);
}
...
static void verify(Container &c) {
    ...
    for (size_t i = 0; i < c.heap.size(); ++i) {
        ...
        assert(l >= c.heap.size() || c.heap[l].val >= c.heap[i].val); // here
        assert(r >= c.heap.size() || c.heap[r].val >= c.heap[i].val);
        assert(*c.heap[i].pos_ref == i);
    }
}
